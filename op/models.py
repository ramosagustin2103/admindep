...
class Deuda(models.Model):
	...
	@property
	def saldo(self):
		total_pagos = sum([p.valor for p in self.deudaop_set.filter(op__confirmado=True, op__anulado__isnull=True)])
		return self.total - total_pagos
	...
	def save(self, *args, **kw):
		if self.retencion and not self.numero:
			numero = 1
			try:
				ultimo = Deuda.objects.filter(club=self.club, retencion=self.retencion).order_by('-numero')[0].numero
			except:
				ultimo = 0
...
class OP(models.Model):
	...
	def hacer_pdf(self):
		if not self.pdf:
			...
			html = HTML(string=html_string, base_url='https://sportingclubsalta.com/pagos/')
			pdf = html.write_pdf()
			ruta = "{}_{}.pdf".format(
