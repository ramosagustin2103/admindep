...
def op_index(request):
	...
	ops = OP.objects.filter(club=club(request), confirmado=True).order_by('-id')
...
def op_vinculaciones(request, pk):
	try:
		op = OP.objects.get(pk=pk, club=club(request), confirmado=False)
	except:
		return redirect('op')
	...
	deudas = Deuda.objects.filter(club=club(request), acreedor=op.acreedor, pagado=False)
			...
			if retenciones:
				...
				for ret in retenciones:
					...
					ret_deuda = Deuda(
						...
						fecha=op.fecha,
						acreedor=Acreedor.objects.get(club=club(request), recibe=tipo_retencion),
						total=ret['valor'],
						)
					ret_deuda.save()
...
class HeaderExemptMixin:
	...
	def get_cierre(self):
		try:
			return Cierre.objects.filter(club=club(self.request), confirmado=True).order_by('-periodo')[0]
		except:
			return None
