...
def deud_index(request):
	...
	deudas = Deuda.objects.filter(club=club(request), confirmado=True).order_by('-id')
...
def deud_nuevo(request):
	...
	if form.is_valid():
		...
		try:
			ultimo_cierre = Cierre.objects.filter(club=club(request), confirmado=True).order_by('-periodo')[0].periodo
		except:
			ultimo_cierre = None
...
def deud_vinculaciones(request):
	...
	erogaciones_posibles = acreedor.tipo.filter(inactivo=False).exclude(cuenta_contable=acreedor.cuenta_contable)
