...
class Bienvenida(models.Model):
	...
	def enviar(self):
		if self.socio.club.mails and not self.fecha_envio:
			socio = self.socio
			if not socio.codigo:
			...
			msg = EmailMultiAlternatives(
				subject="Bienvenido a AdminDEP - Crea tu usuario",
				body="",
				from_email=emisor_mail(self.socio.club),
				to=[self.mail],
			)
			instructivo = Libro.objects.get(club__isnull=True, slug='instructivo-basico-del-socio.pdf')
...
class Transferencia(models.Model):
	...
	def hacer_pdf(self):
		if not self.pdf:
			...
			html = HTML(string=html_string_pdf, base_url='https://sportingclubsalta.com/herramientas/')
			pdf = html.write_pdf()
			ruta = "{}_{}.pdf".format(
