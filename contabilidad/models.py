...
class Cuenta(models.Model):
	club = models.ForeignKey(Club, blank=True, null=True, on_delete=models.CASCADE) # Para cuentas especificas
...
class Ejercicio(models.Model):
	...
	def save(self, *args, **kw):
		if self.activo == True:
			otros_ejercicios = Ejercicio.objects.filter(club=self.club).exclude(id=self.id)
			if otros_ejercicios:
	...
	def delete(self, *args, **kw):
		otros_ejercicios = Ejercicio.objects.filter(club=self.club).exclude(id=self.id).order_by('-cierre')
		if otros_ejercicios:
