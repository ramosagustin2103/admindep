...
def pc_index(request):
	try:
		plan = Plan.objects.get(club=club(request))
	except:
...
def pc_set(request, cuenta):
	...
	try:
		cuenta = Plan.objects.get(club=club(request)).cuentas.get(numero=cuenta)
	except:
		return redirect('contabilidad')
...
def pc_desvincular(request, cuenta):
	...
	cajas = cuenta.caja_set.filter(club=club(request))
	ingresos = cuenta.ingreso_set.filter(club=club(request))
	gastos = cuenta.gasto_set.filter(club=club(request))
	acreedores = cuenta.acreedor_set.filter(club=club(request))
	cuentas_importantes = [
	...
	if cuenta.numero in cuentas_importantes:
		...
		if not cajas and not ingresos and not gastos and not acreedores and not operaciones:
			if cuenta.club:
				...
				Plan.objects.get(club=club(request)).cuentas.remove(cuenta)
