...
def asiento_cierre_res(ejercicio):
	...
	cuentas = Plan.objects.get(club=ejercicio.club).cuentas.filter(nivel=4, numero__gt=400000).order_by('numero')
	asientos = Asiento.objects.filter(fecha_asiento__range=rango)
	sumas = generacionSaldos(cuentas, asientos)

	cuenta_resultados = Cuenta.objects.get(numero=321001)
	total_resultados = sum(sumas.values())
	...
	if not sumas:
		...
		operaciones = [
			...
			) for cuenta,saldo in sumas.items() if saldo !=0
		]
...
def asiento_cierre_pat(ejercicio):
	...
	rango = [ejercicio.inicio, ejercicio.cierre]
	cuentas = Plan.objects.get(club=ejercicio.club).cuentas.filter(nivel=4).order_by('numero')
	asientos = Asiento.objects.filter(fecha_asiento__range=rango)
	sumas = generacionSaldos(cuentas, asientos)

	if not sumas:
		return "No se encontraron asientos en el ejercicio."
...
def asiento_apertura(ejercicio):
	...
	try:
		ejercicio_anterior = Ejercicio.objects.filter(cierre__lt=ejercicio.inicio).order_by('-cierre')[0]
	except:
	...
	if not asiento_cierre_anterior:
		...
		operaciones = [
			...
			) for op in asiento_cierre_anterior.operaciones.all()
		]
...
def asiento_op(op, gastoOP, deudaOP, retencionOP, cajaOP):
	...
	# Creacion del asiento
	fecha = op.fecha_operacion or op.fecha
	asiento = Asiento(
			club=op.club,
			fecha_asiento=fecha,
			descripcion=descripcion,
...
def asiento_liq(liquidacion, creditos):
	operaciones = []
	identificacion = randomNumber(Operacion, 'numero_aleatorio')
	ingresos = Ingreso.objects.filter(club=liquidacion.club)
...
def asiento_liq_auto(cierre, liquidacion):
	...
	ingreso = Ingreso.objects.filter(club=cierre.club, primario=True)[0]
