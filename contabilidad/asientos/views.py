...
def asiento_eliminar(request):
	...
	try:
		ejercicio = Ejercicio.objects.filter(cierre__gte=asiento.fecha_asiento).order_by('cierre')[0]
	except:
		ejercicio = None
	...
	if asiento:
		if asiento.principal:
			...
			if not asiento_principal_posterior:
				try:
					ejercicio_posterior = Ejercicio.objects.filter(inicio__gt=ejercicio.cierre).order_by('inicio')[0]
