...
class Cierre(models.Model):
	...
	total = models.DecimalField(max_digits=20, decimal_places=2, blank=True, null=True) # En desuso
	...
	def enviar_mails(self):
		if self.club.mails:
			...
			socios = self.club.socio_set.filter(baja__isnull=True)
			for socio in socios:
				...
				if socio.usuario:
					if socio.usuario.email:
						msg = EmailMultiAlternatives(
							subject="Nuevos reportes de tu club",
							body="",
