...
class CierreManager(Logo):
	...
	def __init__(
		...
		self.club = cierre.club
		self.cuentas = Plan.objects.get(club=self.club).cuentas.filter(nivel=4)
		self.ejercicio = Ejercicio.objects.get(club=self.club, activo=True)

		self.anteriores = Cierre.objects.filter(club=self.club, confirmado=True, periodo__lt=self.fecha_fin).order_by('-periodo')[:11]
		if self.anteriores:
