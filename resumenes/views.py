...
def res_sp(request):
	try:
		...
		socios = Socio.objects.filter(id__in=request.POST.getlist('socios')).order_by('apellido')
	except:
...
def res_cob(request):
	...
	ingresos = {}
	for i in Ingreso.objects.filter(club=club(request)):
		ingresos.update({i: 0})

	cajas = {}
	for c in Caja.objects.filter(club=club(request)):
		cajas.update({c: 0})
...
def res_pagos(request):
	...
	erogaciones = {}
	for g in Gasto.objects.filter(club=club(request)):
		erogaciones.update({g: 0})

	cajas = {}
	for c in Caja.objects.filter(club=club(request)).exclude(primario=True).order_by('nombre'):
		cajas.update({c: 0})
...
def res_mdc(request):
	...
	if fecha:
		...
		if not caja.fecha or not caja.saldo:
