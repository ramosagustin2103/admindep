...
from .models import *
from parametros.models import Caja
...
class ComprobanteCreator:
	...
	def __init__(
			self,
			data_inicial, # Diccionario LIMPIO que contiene "punto", "socio", "fecha_de_cobro", "condonacion"
			...
			data_mp=None, #Lista de objetos guardados en la base de datos tipo cobro
		):
	...
	def hacer_cobros_y_creditos(self):
		...
		cobros = []
		creditos = []
		#si existe data de cobros por mp
		if self.data_mp:
	...
	def hacer_cajas(self):
		...
		cajas = []
		if self.data_mp:
			caja = Caja.objects.get(club=self.club, primario=True)
			suma = 0
			for data in self.data_mp:
				suma += data.subtotal
	...
	def guardar(self):
		...
		comprobante = Comprobante(
			club=self.club,
			socio=self.socio,
			punto=self.punto if self.tipo == "Recibo X" else None,
			fecha=date.today(),
		...
		if nota_credito:
			...
			validacion = nota_credito.validate()
			if validacion:
				nota_credito.delete()
				return validacion
		...
		if nota_debito:
			...
			validacion = nota_debito.validate()
